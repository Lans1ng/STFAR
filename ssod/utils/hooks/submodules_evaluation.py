...
class SubModulesDistEvalHook(DistEvalHook):
    ...
    def _do_evaluate(self, runner):
        ...
        for submodule in submodules:
            ...
            results = single_gpu_test(runner.model, self.dataloader, False, None,show_score_thr=0.3)
    ...
    def evaluate(self, runner, results, prefix=""):
        ...
        if prefix == 'student':
            setattr(runner, 'eval_res_student', eval_res)
        if prefix == 'teacher':
            setattr(runner, 'eval_res_teacher', eval_res)

        # import ipdb; ipdb.set_trace()
        for name, val in eval_res.items():
            if name == 'categorical_info':
                continue
