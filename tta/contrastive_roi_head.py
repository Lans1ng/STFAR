...
class ContrastiveRoIHead(StandardRoIHead):
    ...
    def _bbox_forward(self, x: List[Tensor], rois: Tensor) -> Dict:
        ...
        cls_score, bbox_pred, bbox_conv_feats, contrast_feat = self.bbox_head(bbox_feats)
        bbox_results = dict(
            cls_score=cls_score,
            bbox_pred=bbox_pred,
            bbox_feats=bbox_feats,
            contrast_feat=contrast_feat,
            bbox_conv_feats=bbox_conv_feats)
        return bbox_results
...
#         if self.with_shared_head:
#             bbox_feats = self.shared_head(bbox_feats)
#         cls_score, bbox_pred ,bbox_conv_feats = self.bbox_head(bbox_feats)

#         bbox_results = dict(
#             cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=bbox_feats, bbox_conv_feats  = bbox_conv_feats)
#         return bbox_results
