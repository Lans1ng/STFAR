...
class IJCAI(SoftTeacher):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(IJCAI, self).__init__(model, train_cfg, test_cfg)
...
#         self.instance_template_cov = [None for _ in range(self.num_classes)]
...
#         self.init_instance_sinkhorn(train_cfg.instance_feature_path)

                
#         self.student.test_cfg.rcnn.score_thr=0.3
...
#             self.register_buffer("recall", torch.zeros(1))
    ...
    def init_instance_distributions(self, feature_path):
        ...
        cov = self.template_instance_cov + cov  # 添加模板协方差（如果需要）

        # 初始化 source distribution 和 target distribution
        ...
        for class_name, features in all_features.items():
            ...
            class_cov = self.template_instance_cov + class_cov  # 添加模板协方差（如果需要）
    ...
    def update_target_distribution(self, backbone_features):
        ...
        for layer in self.layers:
            ...
            #print('layer_features',layer_features.shape)#[2,256]
#             alpha = 1/self.ema_length_cls_global if self.ema_total_n > self.ema_length_cls_global  else 1. / (self.ema_total_n  + 1e-10) #如果长度小于设定的长度，则不需要除以总长度，只需要除以当前长度即可。1/64 for voc
            alpha = 1/self.ema_length #1/64 for voc
#             alpha = 1/256 #1/64 for voc
...
#             regularization = 1e-4  # 可以根据需要调整这个值
